...
int main (int argc, char *argv[])
{
   ...
   for (unsigned int k = 0; k<NFR ; k++) {
	 ...
	 for(unsigned int j=0; j<Npts; j++) {
	    ...
	    const p3vector<double> rij(X[i]- Xcoil[j]);
	    const p3vector<double> n1i(dA_dtdp[i]);
	    const p3vector<double> n2j(dA_dtdp_coil[j]);
	    const double r = norm(rij);
	    
	    M(j) = mu0div4pi * ( 3*(n1i|rij)*(rij|n2j)/pow(r,5.0) - (n1i|n2j)/pow(r,3.0) );
	 }
	 fg[i]=(M|f);
      }
      double temp = 0;
      for (unsigned int a =0; a<Npts; a++) {
	 temp += fg[a].real()*fg[a].real() + fg[a].imag()*fg[a].imag();
      }      
      fgmag[k] = sqrt(temp);
   }
